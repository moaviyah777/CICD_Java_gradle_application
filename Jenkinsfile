            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t 10.128.0.42:8083/springapp:${VERSION} .
                                docker login -u admin -p $docker_password 10.128.0.42:8083 
                                docker push  10.128.0.42:8083/springapp:${VERSION}
                                docker rmi 10.128.0.42:8083/springapp:${VERSION}
                            '''
                    }
                }
            }
